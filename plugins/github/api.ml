...
module Commit_id = struct
  ...
  let pr_fork_branch_name t =
    match t.id with
    | `Ref _ -> None
    | `PR pr -> Some pr.headRefName

  let pr_fork_owner_name t =
    match t.id with
    | `Ref _ -> None
    | `PR pr -> Some pr.headNameWithOwner
end
...
module Refs = Monitor(struct
  ...
  let parse_pr ~owner ~repo json =
    ...
    let headRefName = node / "headRefName" |> to_string in
    let headNameWithOwner = node / "headRepository" / "nameWithOwner" |> to_string in
...
module Commit = struct
  ...
  let pr_fork_owner_name (_, id) = Commit_id.pr_fork_owner_name id
end
